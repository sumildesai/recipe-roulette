...
`.github/workflows/pages.yml` builds and deploys on pushes to `main`. Weekly scheduled and manual runs first regenerate the catalog using the secret, then test, lint, type-check, build, and deploy. The static build infers `/<repository-name>` as the `basePath` for project sites and no base path for `<owner>.github.io` user or organization sites, so assets and `recipes.json` work in either Pages layout. Pull requests use the checked-in seed and never receive the API secret.
