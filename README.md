...
The generator fetches each channel's uploads, normalizes text and durations, excludes short/non-recipe videos, infers meal type, cuisine, and stated cooking time, and writes deterministic JSON to the ignored `public/recipes.local.json` by default. GitHub Actions explicitly directs deployment generation to `public/recipes.json` inside its temporary build workspace. Classification taxonomy now lives in `scripts/classification-taxonomy.ts`, where meal/cuisine aliases are type-checked against supported `MealType` and `Cuisine` values. Cuisine policy is explicit: recognized non-core cuisine aliases map to `Global`, while recipes with no cuisine signal remain unclassified (`null`). Vegetarian classification is intentionally conservative: explicit meat terms produce `false`, clear vegetarian signals produce `true`, and ambiguous recipes remain `null`. Unknown cooking times stay available when the UI has no time cap and are excluded when a cap is active.

Meal types are inferred deterministically first. Recipes without reliable metadata are classified in batches through the GitHub Copilot SDK when `COPILOT_GITHUB_TOKEN` is set. Create a fine-grained personal access token owned by your personal account, grant its account-level **Copilot Requests** permission, and store it as the repository Actions secret `COPILOT_GITHUB_TOKEN`. The token requires an active Copilot subscription and consumes Copilot AI credits. AI responses are cached by recipe metadata and classifier version; scheduled workflows restore and save that cache to avoid spending credits on unchanged recipes. Local generation works without the token and leaves ambiguous meal types unresolved.
...
3. Add a fine-grained personal access token with the account-level **Copilot Requests** permission as the repository Actions secret `COPILOT_GITHUB_TOKEN`.
4. Push to `main`, or run **Build and deploy Recipe Roulette** manually. Enable **Regenerate the catalog from YouTube before deploying** in the manual run to refresh the catalog immediately.
