...
`.github/workflows/pages.yml` builds and deploys on pushes to `main`, scheduled runs every Monday and Friday at 05:17 UTC, and manual runs. Only the scheduled runs regenerate the catalog using the secret, verify it, and save it to the Actions cache. Pushes to `main` and manual runs restore that cached catalog instead of regenerating it, so a deployment never replaces a generated catalog with the seed. A missing key or suspicious seed-sized result fails the scheduled refresh instead of silently publishing the starter catalog, which leaves the previously cached catalog intact for later deployments. The checked-in seed is deployed only until the first scheduled refresh populates the cache. The static build infers `/<repository-name>` as the `basePath` for project sites and no base path for `<owner>.github.io` user or organization sites, so assets and `recipes.json` work in either Pages layout. Pull requests use the checked-in seed and never receive the API secret.
